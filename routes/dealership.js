...
// routes/dealership.js - Add this route
router.get('/disputes', async (req, res) => {
    try {
        const dealershipId = await getDealershipId(req.session.userId);
        const { rows } = await pool.query(`
            SELECT d.*, c.full_name as customer_name, v.vin, v.make, v.model
            FROM disputes d
        ...
        res.render('dealership/disputes', {
            title: 'Customer Disputes',
            disputes: rows,
            unresolvedCount: rows.filter(d => d.status !== 'resolved').length
        });
    } catch (err) {
        console.error(err);
